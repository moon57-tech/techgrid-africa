...
- **1,392 products** across **19 categories** — smartphones, foldables, tablets, rugged & budget phones, plus **400+ home electronics** (TVs, appliances, laptops, audio, gaming, power/load-shedding, accessories), furniture, and a full **fashion range**: women's clothing (305) & footwear (52), men's clothing (155) & footwear (50), kids' clothing (205) & footwear (32).
- **Catalog sources** — electronics/home real products sourced from hificorp.co.za, revibe.co.za and techmarkit.co.za homepages (Aug 2026); fashion modelled on the ackermans.co.za and bash.com ranges. Data lives in `js/products-*.js`, generated by `_scripts/gen-*.js`.
...
- **Pay in 3 segments** — orders over **R35,000** can be split into 3 interest-free monthly segments (0%, ~R0 fees). PayFast charges only segment 1 today; segments 2 & 3 are scheduled (due +30/+60 days). The plan and per-segment status appear on the success page and order card.
...
Orders with a total of **R35,000 or more** get a "Pay in 3 segments" option on checkout (0% interest).

- Three segments; each is `Math.round(total / 3)`, and segment 1 takes the rounding remainder (`total − 2 × seg`) so the three segments always sum exactly to the order total.
- Only **segment 1** is charged now via PayFast (the PayFast amount is `installmentAmount`, and the item name is tagged "(Segment 1 of 3)"). Segments 2 and 3 are scheduled for +30 and +60 days.
- Segment status is tracked in the order: `order.payment.plan = "installments"`, `order.payment.installments[]` with `{n, amount, due, status}`. On PayFast return, segment 1 flips to `Paid`; the plan is shown on the success page and order card.
- Logic lives in `js/store.js` (`PAY_IN_3_FROM`, `canPayIn3`, `payIn3Segments`, `createOrder`) with the UI in `js/app.js` (`installmentBlock`) and CSS in `css/style.css` (`.inst-*`).
...
- The original **94 phone/foldable/tablet products** have real photos hosted on **catbox.moe**, mapped in `js/imgurls.js`, with local copies in `images/<id>.png` as fallback.
- Every other product (1,298 items) gets a **unique, branded per-product SVG** at `images/ph/<id>.svg` — category-coloured card with the product name, brand and a category icon. Generated by `_scripts/gen-product-images.js` (reads the catalog files and renders one SVG per product).
- A global `onerror` fallback in `js/app.js` (`imgFallback`) swaps any image that fails to load for the matching category placeholder (`images/ph/<cat>.svg`).
- The source retailers (hificorp.co.za, techmarkit.co.za, revibe.co.za) render product images via JavaScript, so their homepages expose no stable, hotlinkable catalog photos — the per-product SVGs keep every product's picture correct and self-contained instead.
